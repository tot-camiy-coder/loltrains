...
async def _fetch_routes_data(c0: str, c1: str) -> Dict:
    ...
    tasks = [
        client.get(URLS["PRICES"], params={"service_provider": "B2B_RZD", "origin": c0, "destination": c1, "departureDate": today}),
        client.post(URLS["DEPARTED"], json={"departureExpressCode": c0, "arrivalExpressCode": c1})
    ]
    ...
    for t in departed_data + actual_data.get("Trains", []):
        try:
            ...
            processed_trains.append({
                ...
                "route": f"{t.get('OriginName')} - {t.get('DestinationName')}",
                "ts_dep": dep.isoformat(),
                "ts_arr": arr.isoformat(),
